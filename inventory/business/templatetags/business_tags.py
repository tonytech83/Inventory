...
def count_devices_by_filter(business, filter_query=None):
    if isinstance(business, QuerySet):
        return sum(
            [
                b.device_set.filter(filter_query).count()
                if filter_query
                else b.device_set.count()
                for b in business
            ],
        )

    return (
        business.device_set.filter(filter_query).count()
        if filter_query
        else business.device_set.count()
    )
...
def count_by_status(business, status):
    if isinstance(business, QuerySet):
        return sum(
            [b.device_set.filter(status__icontains=status).count() for b in business],
        )
...
def count_by_category(business, category):
    if isinstance(business, QuerySet):
        return sum(
            [
                b.device_set.filter(category__icontains=category).count()
                for b in business
            ],
        )
